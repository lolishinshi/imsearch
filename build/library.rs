...
impl Library {
    ...
    fn process_env_var_list<'a, T: From<&'a str>>(
        env_list: Option<EnvList<'a>>,
        sys_list: Vec<T>,
    ) -> Vec<T> {
    ...
    pub fn probe_system(
        ...
        let probe_cmake =
            || Self::probe_cmake(include_paths, link_paths, link_libs, None, None, None);
        ...
        let explicit_pkg_config = env::var_os("PKG_CONFIG_PATH").is_some()
            || env::var_os("OPENCV_PKGCONFIG_NAME").is_some();
