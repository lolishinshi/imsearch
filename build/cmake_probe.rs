...
impl<'r> CmakeProbe<'r> {
    ...
    fn extract_from_output(
        ...
        opencv_include_paths: &mut Vec<PathBuf>,
    ) -> Result<()> {
                    ...
                    match name {
                        Some("OCVRS_INCLUDE_DIRS") => {
                            opencv_include_paths.extend(value.split(';').filter_map(|s| {
                                if !s.is_empty() { Some(PathBuf::from(s.trim())) } else { None }
                            }));
                        }
                        Some("OCVRS_VERSION") => {
    ...
    fn extract_from_cmdline(
        ...
        for arg in Shlex::new(cmdline.trim()) {
            ...
            if let Some(path) = arg.strip_prefix("-I") {
                ...
                arg.strip_prefix("-L").or_else(|| arg.strip_prefix("-Wl,-rpath,"))
            {
                let path = PathBuf::from(path.trim_start());
    ...
    fn extract_from_makefile(
        &self,
        link_paths: &mut Vec<PathBuf>,
        link_libs: &mut Vec<String>,
    ) -> Result<()> {
        let link_cmdline =
            fs::read_to_string(self.build_dir.join("CMakeFiles/ocvrs_probe.dir/link.txt"))?;
    ...
    pub fn probe_makefile(&self) -> Result<ProbeResult> {
        ...
        cmd.output().map_err(Box::<dyn std::error::Error>::from).and_then(|output| {
            Self::extract_from_output(&output, &mut version, &mut include_paths)
        })?;
